...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            for p in self.legalPositions:
                self.beliefs[p] *= emissionModel[util.manhattanDistance(p, pacmanPosition)]
        self.beliefs.normalize()
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for p in self.legalPositions:
            ...
            for newPos, prob in newPostDist.items():
                positions[newPos] += prob * self.beliefs[p]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for i in range(self.numParticles):
            ...
            self.particles.append(legalPos[i%len(legalPos)])
    ...
    def observe(self, observation, gameState):
        ...
        pacmanPosition = gameState.getPacmanPosition()
        "*** YOUR CODE HERE ***"
        weightedParticles = util.Counter() #counter object
        if noisyDistance == None:
            for i in range(self.numParticles):
            ...
            for i in range(self.numParticles):
                ...
                weightedParticles[particle] += emissionModel[util.manhattanDistance(particle, pacmanPosition)]
            if weightedParticles.totalCount() == 0:
                self.initializeUniformly(gameState)
            else:
                weightedParticles.normalize()
                for i in range(self.numParticles):
                    self.particles[i] = util.sample(weightedParticles)
        ...
        for i in range(self.numParticles):
            ...
            for j in range(self.numGhosts):
                if noisyDistances[j] == None:
                    ...
                    weight *= emissionModels[j][util.manhattanDistance(pacmanPosition, particle[j])]
